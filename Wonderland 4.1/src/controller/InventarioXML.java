...
public class InventarioXML {
    ...
    private Productos ProductoToObject(Element element) throws ParseException {
        Productos lib = new Productos(element.getChildText("Nombre"), element.getChildText("Descripcion"),
                ...
                element.getChildText("Categoria"), element.getChildText("Serial"),element.getChildText("Proveedor"));
        return lib;
    }
    ...
    public static Element buscar(List raiz, String dato) {
        ...
        while (i.hasNext()) {
            ...
            if (dato.equals(e.getChild("Nombre").getText().toUpperCase())) {
                return e;
            }
        }
        return null;
    ...
    public Productos buscarObj(Productos dato) {
        Element aux = new Element("Producto");
        List Libro = this.root.getChildren("Producto");
        while (aux != null) {
            aux = InventarioXML.buscar(Libro, dato.getNombre().toUpperCase());
            if (aux != null) {
                try {
                    return ProductoToObject(aux);
                } catch (ParseException ex) {
    ...
    public Productos buscarProductoSerial(String dato) {
        Element aux = new Element("Producto");
        List Libro = this.root.getChildren("Producto");
        while (aux != null) {
            aux = InventarioXML.buscarSerial(Libro, dato);
            if (aux != null) {
                try {
                    return ProductoToObject(aux);
                } catch (ParseException ex) {
    ...
    public boolean actualizarProducto(Productos nE) {
        ...
        while (aux != null) {
            aux = InventarioXML.buscar(Producto, nE.getNombre().toUpperCase());
            if (aux != null) {
                Producto.remove(aux);
                resultado = updateDocument();
            }
    ...
    public boolean borrarProducto(String Producto) {
        ...
        while (aux != null) {
            aux = InventarioXML.buscarSerial(Productos, Producto);
            if (aux != null) {
