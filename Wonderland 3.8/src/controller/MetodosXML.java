...
public class MetodosXML {
    ...
    public Usuario buscarPersona(String seudonimo) {
        Element aux = new Element("Metodo");
        List usuarios = this.root.getChildren("Metodo");
        while (aux != null) {
    ...
    public boolean actualizarMetodos(Usuario nE) {
        boolean resultado = false;
        Element aux = new Element("Metodo");
        List Usuario = this.root.getChildren("Metodo");
        while (aux != null) {
    ...
    public static Element buscarMetodo(List raiz, String dato, String tarjeta) {
        Iterator i = raiz.iterator();
        while (i.hasNext()) {
            Element e = (Element) i.next();
            if (dato.equals(e.getChild("seudonimo").getText()) && tarjeta.equals(e.getChild("numTarj").getText())) {
                return e;
            }
        }
        return null;
    ...
    public boolean borrarMetodo(String seudonimo, String tarjeta) {
        boolean resultado = false;
        Element aux = new Element("Metodo");
        List Usuario = this.root.getChildren("Metodo");
        while (aux != null) {
            aux = MetodosXML.buscarMetodo(Usuario, seudonimo, tarjeta);
            if (aux != null) {
                Usuario.remove(aux);
                resultado = updateDocument();
            }
